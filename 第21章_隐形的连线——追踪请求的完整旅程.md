...
%%{init: {'theme':'dark'}}%%
graph LR
    User[用户浏览器] --> LB[负载均衡器<br/>Nginx]
    LB --> SF[Storefront服务<br/>Django]
    SF --> Cache{Redis缓存<br/>命中?}
    Cache -->|未命中| Inv[Inventory服务<br/>库存查询]
    Cache -->|命中| Return1[返回缓存结果]
    Inv --> DB[(PostgreSQL<br/>主库)]
    DB --> Return2[返回查询结果]
    Return2 --> SF
    Return1 --> SF
    SF --> User
...
graph TB
    subgraph Application[应用程序]
        App1[Storefront服务<br/>OpenTelemetry SDK]
        App2[Payment服务<br/>OpenTelemetry SDK]
        App3[Inventory服务<br/>OpenTelemetry SDK]
    end
    
    Agent[Jaeger Agent<br/>本地收集代理<br/>UDP:6831]
    
    Collector[Jaeger Collector<br/>接收和处理Span<br/>HTTP:14268]
    
    Storage[(存储后端<br/>Cassandra/Elasticsearch<br/>或Badger嵌入式)]
    
    Query[Jaeger Query<br/>查询服务<br/>HTTP:16686]
    
    UI[Jaeger UI<br/>Web界面]
    
    App1 -.->|发送Span<br/>UDP| Agent
    App2 -.->|发送Span<br/>UDP| Agent
    App3 -.->|发送Span<br/>UDP| Agent
    
    Agent -->|批量转发| Collector
...
```mermaid
%%{init: {'theme':'dark', 'themeVariables': {'primaryColor':'#1f2937','primaryTextColor':'#f3f4f6','primaryBorderColor':'#60a5fa','lineColor':'#60a5fa','secondaryColor':'#374151','tertiaryColor':'#10b981','noteBkgColor':'#374151','noteTextColor':'#f3f4f6','actorBkg':'#1f2937','actorBorder':'#60a5fa','actorTextColor':'#f3f4f6','actorLineColor':'#60a5fa','signalColor':'#f3f4f6','signalTextColor':'#f3f4f6','labelBoxBkgColor':'#374151','labelBoxBorderColor':'#60a5fa','labelTextColor':'#f3f4f6','loopTextColor':'#f3f4f6','activationBorderColor':'#10b981','activationBkgColor':'#374151','sequenceNumberColor':'#111827'}}}%%
sequenceDiagram
...
%%{init: {'theme':'dark'}}%%
graph LR
    SF[Storefront<br/>1.2K req/min<br/>平均: 234ms]
    Pay[Payment<br/>856 req/min<br/>平均: 145ms]
    Inv[Inventory<br/>1.1K req/min<br/>平均: 89ms]
    DB[(PostgreSQL<br/>2.8K queries/min<br/>平均: 12ms)]
    Redis[(Redis<br/>5.2K ops/min<br/>平均: 2ms)]
    Stripe[Stripe API<br/>856 req/min<br/>平均: 892ms]
...
%%{init: {'theme':'dark'}}%%
graph LR
    SF[Storefront] -->|调用| Inv[Inventory]
    Pay[Payment] -->|调用| Inv
    Analytics[Analytics] -->|调用| Inv
    Inv -->|查询| DB[(PostgreSQL主库)]
...
%%{init: {'theme':'dark'}}%%
graph LR
    User[用户报告问题] --> Loki[Loki日志搜索]
    Loki -->|点击Trace ID| Jaeger[Jaeger追踪详情]
    Jaeger -->|点击Span| Loki_Detail[Loki查看Span日志]
    Loki_Detail -->|点击相关Trace| Jaeger
    
    Jaeger --> Insights[发现瓶颈<br/>识别慢Span]
    Loki_Detail --> Root_Cause[查看详细错误<br/>提取参数]
